...
basestr=mesoDM2D_N"$NCELLS"_n"$n1"_ca"$calA0"_kb0"$kb0"_be"$betaEff"_da"$da0"_dl"$dl0"_P"$P0"_h"$ctch"_cL"$cL"_cB"$cB"
...
binf=bin/"$runstr".o
mainf=$maindir/readInMesoDynMat2D.cpp
...
for f in $flist; do
    ...
    # create output file
    posf=$simdatadir/"$basestr"_seed"$seed".posctc
    hessf=$simdatadir/"$basestr"_seed"$seed".hess

    # create runString
    runString="./$binf $f $kb0 $betaEff $da0 $dl0 $P0 $ctch $cL $cB $seed $posf $hessf"
