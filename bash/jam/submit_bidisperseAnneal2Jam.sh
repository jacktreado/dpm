...
# name strings
basestr=lobes_N"$NCELLS"_n"$nsmall"_calA0"$calA0"_kb"$kb"_trun"$trun"_thK"$T0"
...
binf=bin/"$runstr".o
mainf=$maindir/bidisperseAnneal2Jam.cpp
echo Running bidisperseAnneal2Jam simulations with parameters:
echo NCELLS = "$NCELLS"
echo nsmall = "$nsmall"
echo calA0 = "$calA0"
echo kb = "$kb"
...
for seed in `seq $startSeed $numSeedsPerRun $endSeed`; do
    ...
    for ss in `seq 0 $ssMax`; do
        ...
        # append to runString
        runString="$runString ; ./$binf $NCELLS $nsmall $calA0 $kl $kb $trun $T0 $Ptol $Ftol $runseed $posf"
    done
