// File to test dpm forces using NVT protocol (Langevin integrator)
// 
// Compilation command:
// g++ -O3 --std=c++11 -I src main/test/dpmNVTtest.cpp src/*.cpp -o test.o
...
int main(){
	...
	double phi0 = 0.6, dphi0 = 5e-3, calA0 = 1.05, smallfrac = 0.5, sizefrac = 1.4, Ftol = 1e-10, Ptol = 1e-6, dt0 = 2e-2;
	...
	configobj2D.vertexJamming2D(forceUpdate,Ftol,Ptol,dt0,dphi0,0);
	...
	configobj2D.vertexLangevinNVT2D(enout, forceUpdate, T0, gam, dt0, NT, NPRINTSKIP);

	// say goodbye
	cout << "\n\n** Finished dpmNVTtest.cpp, ending. " << endl;
